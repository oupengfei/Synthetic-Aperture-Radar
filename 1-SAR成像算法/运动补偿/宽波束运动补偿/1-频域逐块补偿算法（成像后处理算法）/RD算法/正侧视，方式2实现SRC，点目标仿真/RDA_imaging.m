...
% 沿地距 x 轴的运动误差
a = 20;
w = 0.8;     % 这个控制着正弦误差是属于低频误差，还是高频误差
% w < 1，是低频误差
% w > 1，是高频误差
% delta_x_t = a*sin(2*pi*w/La*Vr.*ta_mtx);    % 这是沿地距 x 轴的运动误差
delta_x_t = (1*ta_mtx+1*ta_mtx.^2)/70*20;
% delta_x_t = 0;
% 沿z轴（载机平台高度）的运动误差
...
% ================================================================
% 尝试 1 ——这部分有问题
fa_mtx = fa.'*ones(1,N_rg);
Naz_num = Naz/32;
N_num = 2*Naz_num - 1;
s_MOCO = zeros(Naz,N_rg);
h = waitbar(0,'频域逐块补偿，Please wait');
for p = 1:N_num
    s_N_num = zeros(Naz,N_rg);
    s_N_num(1+(p-1)*16:16+16*p,:) = s_ac(1+(p-1)*16:16+16*p,:);
    fa_mtx_num = circshift(fa_mtx,-(Naz/2+1 - 16*p),1);
    delta_Rv_p_fa = r.*(sqrt((1+delta_r./r).^2 +...
        (lamda^2/(4*Vr^2)).*fa_mtx_num.^2./(1-(lamda^2/(4*Vr^2)).*fa_mtx_num.^2))...
        - sqrt(1+(lamda^2/(4*Vr^2)).*fa_mtx_num.^2./(1-(lamda^2/(4*Vr^2)).*fa_mtx_num.^2)))...
        -delta_r;
    S_N_num = fft(s_N_num,[],1);
    S_N_num_MOCO = S_N_num.*exp(1j*4*pi/lamda.*delta_Rv_p_fa);
    s_N_num_MOCO = ifft(S_N_num_MOCO,[],1);
    s_MOCO(1+(p-1)*16:16+16*p,:) = s_N_num_MOCO(1+(p-1)*16:16+16*p,:);
    waitbar(p/N_num);
end
close(h);
figure;
...
for p = 1:N_num
    s_N_num = zeros(Naz,N_rg);
    s_N_num(1+(p-1)*16:16+16*p,:) = s_ac(1+(p-1)*16:16+16*p,:);
    delta_r_x_r = zeros(Naz,N_rg);
    if (16*p - La_num/2) < 1
        delta_r_x_r(1:16*p+La_num/2-1,:) = delta_r(1:16*p+La_num/2-1,:);
    else
        if (16*p+La_num/2-1) > Naz
            delta_r_x_r(16*p-La_num/2:end,:) = delta_r(16*p-La_num/2:end,:);
        else
            delta_r_x_r(16*p-La_num/2:16*p+La_num/2-1,:) = delta_r(16*p-La_num/2:16*p+La_num/2-1,:);
        end
    end
    delta_R_p_x = sqrt( (r+delta_r_x_r).^2 + ((azi_num-16*p)*ones(1,N_rg).*(1/Fa*Vr)).^2  )...
        - sqrt( r.^2 + ((azi_num-16*p)*ones(1,N_rg).*(1/Fa*Vr)).^2  );
    delta_Rv_p_x = delta_R_p_x - delta_r_x_r;
 
    S_N_num = fft(s_N_num,[],1);
    phy_delta_Rv = exp(1j*4*pi/lamda.*delta_Rv_p_x);
    PHY_delta_Rv = fft(phy_delta_Rv,[],1);
    S_N_num_MOCO = S_N_num.*PHY_delta_Rv;
    s_N_num_MOCO = ifft(S_N_num_MOCO,[],1);     

    s_MOCO(1+(p-1)*16:16+16*p,:) = s_N_num_MOCO(1+(p-1)*16:16+16*p,:);
    waitbar(p/N_num);   
    clear delta_R_p_x;clear delta_Rv_p_x;
    clear S_N_num;
    clear phy_delta_Rv;clear PHY_delta_Rv;
    clear S_N_num_MOCO;clear s_N_num_MOCO;
end
close(h);
figure;
imagesc(abs(s_MOCO));
...
% target_1 = target_analysis( s_MOCO(tg_1_x-NN:tg_1_x+NN,tg_1_y-NN:tg_1_y+NN),Fr,Fa,Vr);
