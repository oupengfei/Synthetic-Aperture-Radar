...
% 基于频域分割的运动补偿算法
%
% 2014.11.26. 14:54 p.m. 仿真结果不正确，没有找到方法 
...
% --------------------------------------------------------------------
N_look = 20;        % 频域分块数量 
w1 = ones(Naz/N_look,N_rg);     % 可以在分块时使用加权。这里没有加权。
fa_widebeam_MOCO = fa;
s_look_widebeam_MOCO = zeros(Naz,N_rg);     % 用来存放这部分的处理结果

h = waitbar(0,'频域分割的运动补偿，Please wait');
for p = 1 : N_look
    S_look_p = zeros(Naz,N_rg); % 用来存放每一个取出的子频谱，用于后面运算
    S_look_p(1+Naz/N_look*(p-1):Naz/N_look*p,:) = w1.*S_rd_rcmc_MoCo(1+Naz/N_look*(p-1):Naz/N_look*p,:);
    s_look_p = ifft(S_look_p,[],1); % 将子频谱变换到时域
    
    fi_p = fa_widebeam_MOCO(Naz/N_look*(1/2)+1);% 每一子视（子频谱）的频谱中心为fi_p
    theta_i = asin(lamda*fi_p/(2*Vr));      % 子频谱对应的时域斜视角为theta_i
    delta_r_si = r.*(sqrt((1+delta_r./r).^2 + tan(theta_i)^2) - sqrt(1+tan(theta_i)^2))-delta_r;% 计算方法1
%     delta_r_si = delta_r*cos(theta_i)-delta_r;    % 计算方法2
    Hi = exp(1j*4*pi/lamda.*delta_r_si);
    
    s_look_p_widebeam_MOCO = s_look_p.*Hi;
    % 完成粗补偿处理后，二维空变残留误差的校正
    % 此时位于二维时域
    
    s_look_widebeam_MOCO = s_look_widebeam_MOCO + s_look_p_widebeam_MOCO;
...
% 作图
figure;
imagesc(abs(s_look_widebeam_MOCO));
title('完成“基于频域分割的运动补偿算法”后，二维时域');

% 下面变换到距离多普勒域，准备进行方位压缩
S_look_widebeam_MOCO = fft(s_look_widebeam_MOCO,[],1);  

figure;
imagesc(abs(S_look_widebeam_MOCO));
title('完成“基于频域分割的运动补偿算法”后，距离多普勒域');
...
Haz = exp(1j*4*pi.*(D_fn_Vr*R0_RCMC).*f0./c);   % 方位向MF
% 这里要注意，生成的MF的零频既不是在两端，也不是在中心的。
% 考虑下频率轴是什么样的，间断点在哪里。注意fa的构成。
% 这里的频率轴和距离多普勒域的方位频谱是对应的。

S_rd_c = S_look_widebeam_MOCO.*Haz; % 乘以匹配滤波器
