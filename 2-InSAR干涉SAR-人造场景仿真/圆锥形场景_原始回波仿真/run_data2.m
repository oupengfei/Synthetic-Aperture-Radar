...
% 设计一个圆锥形场景
% 以目标1（场景中心的点目标）为中心，总大小为：1300m*1000m 的矩形
% 其中，圆锥的半径为：400m
%      圆锥中心（即场景中心）的高度为：350m
r_cone = 400;           % 圆锥半径，400m
r_cone_Height = 350;     % 圆锥中心的高度，350m
x_max = 1300;         	% 整个场景沿 x 轴方向的最大范围，1300m。
y_max = 1000;         	% 整个场景沿 y 轴方向的最大范围，1000m。
...
%
% 仿真“双天线单航过干涉SAR”中所用的人造场景原始数据
% 天线 B 的仿真结果（天线A作为参考天线）
%
% 截止到 2014.12.17. 8:49 a.m.
...
load A_phase;       % 载入在天线 A 的原始数据生成时，产生的随机相位
% h = waitbar(0,'Generating Raw Data, Please wait');
for kk = 1:num_target_x                 % 生成k个目标的原始回波数据
    for ll = 1:num_y_azimuth 
        R_n = sqrt( (B + delta_x_t-target_x(kk)).^2 + (Vr.*ta_mtx-y_azimuth(ll)).^2 ...
            + ((H-target_z(kk,ll)).*ones(Naz,Nrg)+delta_z_t).^2 );    % 目标k的瞬时斜距    
