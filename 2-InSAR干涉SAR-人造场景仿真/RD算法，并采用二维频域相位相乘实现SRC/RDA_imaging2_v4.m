function [s_ac,R0_RCMC,Parameter] = RDA_imaging2_v4(raw_data_type)
% 对函数   RDA_imaging_v3（）    进行了以下修改：
%   在RCMC插值时，不采用循环移位的方式。对于溢出部分，直接赋零值；
% 将以上修改后的该版本记为：RDA_imaging2_v4（）
%
% 本函数用来对生成的原始数据成像
% 利用：
...
%%
% 载入原始数据
load Rawdata_Parameter; flag = 0;
% load raw_data5_A_B_After_Filtering; flag = 1;

if raw_data_type == 1
    if flag == 0
        s_echo = sr1;           % 载入天线A对应的原始数据
    end
    if flag == 1
        s_echo = s_echoA_filter;    % 预滤波后使用该数据
    end
end
if raw_data_type == 2
    if flag == 0
        s_echo = sr2;           % 载入天线B对应的原始数据
    end
    if flag == 1
        s_echo = s_echoB_filter;    % 预滤波后使用该数据
    end
end
...
%                 ll = [ll_1,ll_2];
                rcmc_S_rd = [S_rd(p,ll_1),zeros(1,R-length(ll_1))];
            else
...
%                         ll = [ll_1,ll_2];
                        rcmc_S_rd = [zeros(1,R-length(ll_2)),S_rd(p,ll_2)];
                    else
